...
【重构概要】某个继承体系同时承担了两项责任，建立两个继承体系，并通过委托关系让其中一个可以调用另外一个；

【重构动机】继承虽然可以明显减少代码的数量，但是如果将一个特定的问题分散到各个子类中，那么混乱的继承关系将会是致命性的。要指出继承体系是否承担了两项不同的责任并不困难：如果继承体系中的某一个特定层级上的所有类，其子类名称都以相同的形容词开始，那么这个体系很可能就承担着两种不同的责任；
...
# 8 IntelliJ IDEA重构快捷键

|        重构操作        |        Windows         | MacOS |
| :--------------------: | :--------------------: | :---: |
|        重构菜单        |        Alt + R         |       |
| 在光标出调出重构子菜单 | Ctrl + Alt + Shift + T |       |
|         重命名         |       Shift + F6       |       |
|      更改函数签名      |       Ctrl + F6        |       |
|         静态化         |      Alt + R + T       |       |
|         复制类         |           F5           |       |
